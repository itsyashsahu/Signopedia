...
const ListItem = (item: String, isSelected: boolean = false) => {
    if (isSelected)
        return (
            <li>
                <Link href={`/${item.toLowerCase().replace(" ", "")}`} className="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">{item}</Link>
            </li>
        )
    else {
        return (
            <li>
                <Link href={`/${item.toLowerCase().replace(" ", "")}`} className="block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-white dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">{item}</Link>
            </li>
        )
    }
}
        ...
        <nav className="z-20 border-gray-200 px-16 sm:px-24 py-4 rounded h-32 flex justify-center ">
            <div className="container flex flex-wrap items-end  mx-auto">
                <div className='h-12 flex'>
                    ...
                    <Link href='/' className="self-center text-xl pl-4 font-semibold whitespace-nowrap dark:text-white">Signopedia</Link>
