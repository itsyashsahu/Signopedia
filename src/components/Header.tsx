...
const ListItem = (item: String, isSelected: boolean = false) => {
    if (isSelected)
        ...
        return (
            <li>
                <Link href={`/${item.toLowerCase().replace(" ", "")}`} className="whitespace-nowrap block py-2 pl-3 pr-4 rounded md:p-0 text-gray-300 md:hover:text-white">{item}</Link>
            </li>
        )
    }
}
...
const Header = () => {
    return (
        <nav className="z-20 border-gray-200 px-6 md:px-24 py-4 rounded md:h-32 h-16 flex justify-center ">
            <div className="container flex flex-wrap items-end  mx-auto">
                <div className='h-12 flex'>
                    ...
                    <Link href='/' className="self-center md:text-xl text-md  pl-4 font-semibold whitespace-nowrap dark:text-white">Signopedia</Link>

                    <div className="items-center justify-between md:mt-1 mt-2 pl-6 md:pl-16 w-full md:flex md:w-auto">
                        <ul className="flex px-2 md:px-4 rounded-lg  md:space-x-8 md:mt-0 text-xs md:text-sm md:font-medium md:border-0 dark:border-gray-700">
